...
render_view :: proc(r: ^Render_Context, v: View, origin: [2]f32, size: [2]f32) {
	...
	case View_Text_Input:
		...
		_, lh := measure_text_ctx(r, display, vv.font_size, vv.font)
		ascent := text_ascent_ctx(r, vv.font_size, vv.font)
		...
		if !vv.multiline {
			...
			if vv.focused && vv.selection_anchor != vv.cursor_pos && len(vv.text) > 0 {
				...
				x_lo: f32 = 0
				x_hi: f32 = 0
				if lo > 0 {x_lo, _ = measure_text_ctx(r, vv.text[:lo], vv.font_size, vv.font)}
				if hi > 0 {x_hi, _ = measure_text_ctx(r, vv.text[:hi], vv.font_size, vv.font)}
			...
			if vv.focused {
				...
				if vv.cursor_pos > 0 && vv.cursor_pos <= len(vv.text) {
					cw, _ = measure_text_ctx(r, vv.text[:vv.cursor_pos], vv.font_size, vv.font)
				}
				caret_w: f32 = 1.5
			...
			if vv.show_clear {
				...
				gw, glh := measure_text_ctx(r, "×", vv.font_size, vv.font)
				gx := ix + iw + (clear_w - gw) / 2
				gy := iy + (ih - glh) / 2
				ascent := text_ascent_ctx(r, vv.font_size, vv.font)
				draw_text_ctx(r, "×", gx, gy + ascent, glyph_col, vv.font_size, vv.font)
			}
		} else {
			...
			if len(vv.text) == 0 {
				...
				if len(display) > 0 && display_col.a != 0 {
					draw_text_ctx(r, display, ix, iy + ascent, display_col, vv.font_size, vv.font)
				}
				if vv.focused {
				...
				for vl, vli in vv.visual_lines {
					...
					if has_sel && sel_hi > i && sel_lo <= j {
						...
						x_lo: f32 = 0
						x_hi: f32 = 0
						if lo > i {x_lo, _ = measure_text_ctx(r, vv.text[i:lo], vv.font_size, vv.font)}
						if hi > i {x_hi, _ = measure_text_ctx(r, vv.text[i:hi], vv.font_size, vv.font)}
					...
					if vv.focused && vv.cursor_pos >= i && vv.cursor_pos <= j {
						...
						if draw_here {
							cw: f32 = 0
							if vv.cursor_pos > i {
								cw, _ = measure_text_ctx(r, vv.text[i:vv.cursor_pos], vv.font_size, vv.font)
							}
